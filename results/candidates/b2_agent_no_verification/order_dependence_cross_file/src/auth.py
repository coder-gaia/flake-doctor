...
def logout(user_id) -> None:
    """Remove a single user's session from the shared session store."""
...
def reset_sessions() -> None:
    ...
    Exposed so that callers (notably the test suite) can guarantee a clean
    session state instead of inheriting whatever a previous caller left behind
    in the module-level ``_SESSIONS`` dict.
    """
    _SESSIONS.clear()
